...
%% SUBPLOT CONTROL
function subplot_pb_Callback(hObj, ed, h) %#ok<*INUSD>

switch hObj.String
    case 'Choose subjects'
        h.C=Active_subplot_subj2(h.C);
    case 'Choose ROIs'
        h.C=Active_subplot_roi2(h.C);
    case 'Choose space'
        h.C = Active_subplot_model(h.C,h.figure1.Position);
    case 'Choose conn'
        [h.mask_comp_name, h.mask_comp] = Active_subplot_conn3(h.C);
...
for i1 = 1:length(h.pumtag)
    unpack = GetUnpack(h.C);
    h.(h.pumtag{i1}).String = unpack.menu;
...
%% GetUnpack gives you a Behavioral info from the first subject to be used
%in menus and trial type selection
function unpack = GetUnpack(S)
    if strcmp(S.tt.mode,'rs_pair')||strcmp(S.tt.mode,'uni')||strcmp(S.tt.mode,'rs_pair_runNorm')
        unpack = Unpack(S,'rs_pair');
    else
        unpack = Unpack(S,'rs_all');
    end
end
