function SL_warp(C,beta_row,slrad,maskType,repSubj)
...
for subInd = 1: length(subj2run)
    ...
    for i = 1:length(statimages)
        eval(['!',antsDir,'/bin/WarpImageMultiTransform 3 ',fullfile(slDir,subj,statimages(i).name),' ',fullfile(slDir,subj,warpedfold),'/w_',statimages(i).name ' -R ',mmF_rS,' --use-NN ',rDir_cS,'/abWarp.nii ',rDir_cS,'/abAffine.txt'])
    end
...
%% Warp masks
 ...
 subj_masks= dir(fullfile(maskDir,subj,'/','r*.nii'));
        for mas = 1:length(subj_masks)
            eval(['!',antsDir,'/bin/WarpImageMultiTransform 3 ',mDir_cS,'/',subj_masks(mas).name,' ',rDir_cS,'/w_',subj_masks(mas).name,' -R ',mmF_rS,' --use-NN ',rDir_cS,'/abWarp.nii ',rDir_cS,'/abAffine.txt'])
        end
        
 %% Warp megamasks     
        
    eval(['!',antsDir,'/bin/WarpImageMultiTransform 3 ',mmF_cS,' ',rDir_cS,'/w_SL_ROI_reg.nii',' -R ',mmF_rS,' --use-NN ',rDir_cS,'/abWarp.nii ',rDir_cS,'/abAffine.txt'])
