...
format = '.txt';
fileName = fullfile(C.dir.dat_behav,subj,[subj,format]);
tt = tdfread(fileName);
clear C fileName 
...
clear S
tt= Get_tt(tt,id,numTrials);

%% tt_all
tt_all.rs_pair = tt;
tt_all.rs_all = GetRS(tt,numTrials);
tt_all.spm = GetSPM(tt,numTrials,dur);%%ttspm;
...
function tt = Get_tt(tt,id,numTrials)
...
function ttspm = GetSPM(tt,numTrials,dur)
        for i1=1:numTrials;
...
function tt  = GetRS(tt,numTrials)
pairs = nchoosek(1:numTrials,2);
tt.ind_non_pair = abs(pairs(:,1) - pairs(:,2)) ~=1;
for i = 1:size(pairs,1)
    x = pairs(i,1);
    y = pairs(i,2);
    
    tt.rs_withinrun(i,1)  = tt.run_num(x) == tt.run_num(y);
    tt.rs_pair_code(i,1)  = abs(tt.currCity(x) - tt.currCity(y));
    tt.run1(i,1)  = tt.run_num(x) ==0 &   tt.run_num(y) ==0;
    tt.run2(i,1)  = tt.run_num(x) ==1 &   tt.run_num(y) ==1;
    tt.run3(i,1)  = tt.run_num(x) ==2 &   tt.run_num(y) ==2;
    tt.run4(i,1)  = tt.run_num(x) ==3 &   tt.run_num(y) ==3;
    
    tt.rs_tt_same(i,1) = tt.tt_code(x) ==0 &   tt.tt_code(y) ==0;
    tt.rs_tt_diff1(i,1) = tt.tt_code(x) ==1 &  tt.tt_code(y) ==1;
    tt.rs_tt_diff2(i,1) = tt.tt_code(x) ==2 &  tt.tt_code(y) ==2;
    tt.rs_tt_diff12(i,1) =  any([tt.rs_tt_diff1(i),tt.rs_tt_diff2(i)]);
    tt.rs_TargC(i,1) = tt.cityTargC{x} == 'T'| tt.cityTargC{y} == 'T';
    tt.rs_MorphC(i,1) = tt.cityTargC{x} == 'M'| tt.cityTargC{y} == 'M';
    tt.rs_NovC(i,1) = tt.cityTargC{x} == 'N'| tt.cityTargC{y} == 'N';
    tt.rs_acc_all(i,1) = tt.acc(x) == 1 & tt.acc(y) == 1;
    tt.rs_acc_or(i,1) = tt.acc(x) == 1 | tt.acc(y) == 1;
end
