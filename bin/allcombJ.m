...
cellarray= varargin;
celllen = nargin;
if celllen == 1 && isstruct(cellarray{1})
 cellarray = struct2cell(varargin{1})';
  celllen = length(cellarray);
else
...
if ischar(cellarray{end}) && (strcmpi(cellarray{end},'matlab') || strcmpi(cellarray{end},'john')),
...
if any(cellfun('isempty',cellarray(ii))),
    warning('ALLCOMB:EmptyInput','Empty inputs result in an empty output.') ;
    A = zeros(0,NC) ;
elseif NC > 1 
    isCellInput = cellfun(@iscell,cellarray) ;
    if any(isCellInput)
        if ~all(isCellInput)
        ...
        ix = cellfun(@(c) 1:numel(c), cellarray,'un',0) ;
        ...
        for k=1:NC,
            % combine
            A(:,k) = reshape(cellarray{k}(ix{k}),[],1) ;
        end
    else
        % non-cell input, assuming all numerical values or strings
        % flip using ii if last column is changing fastest
        [A{ii}] = ndgrid(cellarray{ii}) ;
        % concatenate
        A = reshape(cat(NC+1,A{:}),[],NC) ;
    end
elseif NC==1,
    A = cellarray{1}(:) ; % nothing to combine
