function rsj_spm_model_standard(subj,C)
...
matlabbatch{curRun}.spm.util.exp_frames.files = {fullfile(dir_mri,subj,[spm_funcFold,num2str(curRun)],[spm_smooth,spm_funcFile,num2str(curRun),'.nii'])};
...
%     matlabbatch{specNum}.spm.stats.fmri_spec.sess(curRun).multi = {fullfile(dir_analysis ,spm_modelName,subj,['WW_perf_run',...
%                                                                     num2str(curRun), '_regs.mat'])};
    matlabbatch{specNum}.spm.stats.fmri_spec.sess(curRun).multi = {fullfile(dir_analysis ,spm_modelName,subj,[subj,'_Run',num2str(curRun), '.mat'])}; 
    matlabbatch{specNum}.spm.stats.fmri_spec.sess(curRun).regress = struct('name', {}, 'val', {});
    matlabbatch{specNum}.spm.stats.fmri_spec.sess(curRun).multi_reg = {fullfile(dir_mri,subj,[spm_funcFold,num2str(curRun)],[spm_motionFile,num2str(curRun),'.txt'])};
...
matlabbatch{specNum}.spm.stats.fmri_spec.mask = {fullfile(dir_mask,maskType,subj,'super_mask.nii,1')};
