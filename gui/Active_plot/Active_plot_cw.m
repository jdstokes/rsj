function varargout = Active_plot_cw(varargin)
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @Active_plot_cw_OpeningFcn, ...
    'gui_OutputFcn',  @Active_plot_cw_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
%% Open Plot
function Active_plot_cw_OpeningFcn(hObj, ed, h, varargin)
h = SetupStudy(h,1);
h.output = hObj;
guidata(hObj, h);
end
%% Output
function varargout = Active_plot_cw_OutputFcn(hObj, ed, h) %#ok<*INUSL>
...
axes(h.axes1);
hold off
RSA_group(h.C,h.CI_comp,h.stat_mode,h.measure_func,h.listboxComp.String(2:end,1));
guidata(hObj, h);
end
%% GetConnectivity
...
RSA_group_corr(h.C,h.CI_comp,h.stat_mode,h.mask_comp,h.mask_comp_name,h.measure_func,h.listboxComp.String(2:end,1));
guidata(hObj, h);
end
...
% RSA_group_cum(h.C,h.CI_comp,h.stat_mode,h.measure_func,h.listboxComp.String(2:end,1));
RSA_group_cum_cw(h.C,h.CI_comp,h.listboxComp.String(2:end,1))
guidata(hObj, h);
...
%% ChangeMeasure
function ChangeMeasure(ind,C)
C.rs.rs_calcCurr = ind;
end
...
for i1 = 1:length(h.pumtag)
    if h.C.tt.tt_val == 1
     h.(h.pumtag{i1}).String = h.C.tt.tt.menu;
    else
     h.(h.pumtag{i1}).String = h.C.tt.tt.menu_rs;
    end
...
function pushbutton24_Callback(hObj, ed, h)
end

function pushbutton25_Callback(hObj, ed, h)
end
