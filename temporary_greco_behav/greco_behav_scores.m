...
foldpath = '/Users/jdstokes/Dropbox/Data/dGR/behav/behav_scan/';
...
for curSubj = 1:length(subjects)
    ...
    col.col_resp =  find(~cellfun(@isempty,strfind(HDRS,'resp_key')));
...
for curtr = 1: length(DATA{1})
    ...
    if(~strcmp(DATA{col.col_resp}(curtr),'nan')), code.resp(curtr)     = 1;end
    %correct trials
...
function [score,tt_size]=CalcScores(score_name,code)
