...
numTrials = tt_all.numTrials; clear tt_all;
 ...
 %Skip masks that are empty
    ...
    if ~isempty(betas_masked{i})
        ...
        if size(betas_masked{i},2)>4%Mask must have at least 5 voxels
            ...
            scores{i}.rs = 1-pdist(betas_masked_OLSv{i}(:,ind),'correlation')';
            scores{i}.rsz = real(atanh(scores{i}.rs));
            
            %Uni
            scores{i}.uni_m =  mean(betas_masked_OLSv{i}(:,ind),2);
            scores{i}.uni_sd = std(betas_masked_OLSv{i}(:,ind),0,2);
